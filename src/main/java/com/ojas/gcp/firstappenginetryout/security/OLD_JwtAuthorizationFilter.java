...
public class OLD_JwtAuthorizationFilter extends BasicAuthenticationFilter {
    private AppUserRepository userRepository;

    public OLD_JwtAuthorizationFilter(AuthenticationManager authenticationManager, AppUserRepository userRepository) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        String header = request.getHeader(OLD_JwtUtil.HEADER_STRING);

        // If header does not contain BEARER or is null delegate to Spring impl and exit
        if (header == null || !header.startsWith(OLD_JwtUtil.TOKEN_PREFIX)) {
    ...
    private Authentication getUsernamePasswordAuthentication(HttpServletRequest request) {
        String token = request.getHeader(OLD_JwtUtil.HEADER_STRING)
                .replace(OLD_JwtUtil.TOKEN_PREFIX,"");

        if (token != null) {
            // parse the token and validate it
            String email = JWT.require(HMAC512(OLD_JwtUtil.SECRET.getBytes()))
                    .build()
                    .verify(token)
                    .getSubject();
